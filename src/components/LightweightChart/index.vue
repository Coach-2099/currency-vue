...
const visibleEMAData = computed(() => {
  return Array.from(emaSeriesMap.value.values())
    .map((series) => {
  ...
  configs.forEach((config) => {
    ...
    newCandles.forEach((candle, index) => {
      ...
      if (existingData.length + index < config.period - 1) {
        const closes = props.data.slice(0, existingData.length + index + 1).map((d) => d.close)
...
const initChartStructure = () => {
  ...
  `
  console.log('获取容器 container', container)

  if (container && container.appendChild) {
    container.appendChild(toolTipEl)
  } else {
    console.error('容器元素不存在appendChild方法')
  }
...
const visibleEMAs = computed(() =>
  (props.emaConfigs || defaultEMAConfigs).filter(
    (c) => emaSeriesMap.value.get(c.period)?.options().visible !== false
  )
)
